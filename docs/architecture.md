...
flowchart TD
  ...
  gh -->|push or pull request| ci[GitHub Actions - sixty-one tests against service containers]
  build --> app
...
| **v1.9** | **25** | **SSRF guard at delivery time, failing closed by default; `GET /api/v1/merchants/me`; signup moved off the entity onto `CreateMerchantRequest`, closing a privilege escalation that let an unauthenticated POST grant itself ADMIN; `Retry-After` on 429s.** |