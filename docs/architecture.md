...
## Current: v1.9 — the SSRF guard and the signup escalation (Days 13–25)
...
**The SSRF guard (v1.9):** `WebhookTargetValidator` runs immediately before the POST. It resolves the host and refuses loopback, link-local (`169.254.0.0/16`, where cloud instance metadata lives), site-local, the wildcard address, multicast and IPv6 unique-local — and refuses a host that does not resolve at all, because a destination you cannot determine is not one you send to. The check is at DELIVERY rather than at write, and that is the whole point: DNS rebinding means a hostname resolving to a public address when the URL is saved can resolve to `127.0.0.1` when it is called, so a write-time check is a delay rather than a control. `WEBHOOK_ALLOW_PRIVATE_TARGETS` disables it for local development and defaults to **false**, so a deploy that forgets the variable blocks private targets rather than allowing them. A refused target is skipped with a warning, never thrown on: a blocked address is permanent, so retrying and dead-lettering would spend the whole retry budget on something that can never succeed.

**Login rate limiting (v1.9):** `POST /api/v1/auth/login` is counted by client address rather than by merchant, because the per-merchant limiter keys on an id that only exists after authentication — and the point of attacking login is not having a credential yet. Until Day 25 the endpoint had no ceiling and passwords could be guessed as fast as the network allowed. Five attempts a minute by default against twenty for the API: generous for a human typing a password, useless for a script. The count happens in the filter, before the controller, so failed attempts count too — a limiter that only counted successful logins would be no obstacle to guessing. The address comes from `CF-Connecting-IP` where present, falling back to `getRemoteAddr()`: behind a proxy that fallback is the proxy, which would make the ceiling global rather than per client, and `X-Forwarded-For` is the wrong alternative because it is appended to and its leftmost entry is whatever the caller claimed. It fails open on a Redis outage for the same reason the API limiter does, and because failing closed would lock every human out of the dashboard.
...
**The test suite (v1.1, seventy tests since v1.9):** fifty-three integration tests plus seventeen unit tests across four classes, running against the full stack in about twenty-five seconds since the Day 23 config fix. `AuthIntegrationTest` covers signup, login and both failure paths. `OwnershipIntegrationTest` covers foreign payments and ledgers returning 404, list scoping, and the forged `merchantId` being ignored. `IdempotencyIntegrationTest` covers key reuse, key scoping per merchant, and the deliberate decision not to fingerprint the request body. `RateLimitIntegrationTest` covers the 429 threshold and the fact that one merchant hitting the ceiling does not affect another. `RetentionSweeperIntegrationTest` covers both retention windows and, most importantly, that an unpublished outbox row survives the sweep however old it is. `MerchantWebhookIntegrationTest` covers setting and clearing the webhook URL, the rejected scheme, and both routes by which a merchant might try to reach another merchant's row. `WebhookTargetValidatorTest` pins the SSRF guard using literal IP addresses only, so it performs no DNS lookup and cannot go flaky. `MerchantSignupSecurityIntegrationTest` pins the privilege escalation fixed in v1.9, and does it twice: once on what the response says, and once on what the resulting credential can actually do. `MerchantAdminUpdateIntegrationTest` covers the admin edit route, and is the only test in the suite that needs an ADMIN — signup can no longer produce one, so it creates a merchant, promotes the row through the repository, and only then logs in. That order is load-bearing: the JWT carries the role as a claim and is signed at login, so a token minted before the promotion would still say MERCHANT until it expired. `MerchantAdminDeleteIntegrationTest` does the same for the delete route, and asserts the row is actually gone rather than only that the status was 204 — the status alone would have passed against the old code. Most use `MockMvc`, which sends real requests through the entire filter chain and into a real database without opening a network port.
...
`WebhookConsumerTest` is the project's third unit test, and a unit test for the same reason as the first two: the rule worth pinning needs a dependency to **fail on demand**. `MockRestServiceServer` binds to the `RestClient.Builder` and answers from a script, so request building and serialisation are real while the network is not. Three rules are pinned: a merchant with no `webhookUrl` produces no HTTP call at all; a merchant with one receives the exact payload, as JSON, at exactly that URL; and a merchant server returning 500 makes `deliver` throw. The third earns its place — a `try`/`catch` inside the listener would read as good hygiene in a diff and would silently delete every failed webhook, because `DefaultErrorHandler` only fires on an exception. The dead-letter path itself is still unpinned: the proof is a manual run, and manual runs do not execute in CI.
...
- **The login limiter depends on resolving the client address correctly.** It prefers `CF-Connecting-IP`, which Cloudflare overwrites, and falls back to `getRemoteAddr()`. Behind a proxy that fallback is the *proxy's* address, which would make the five-a-minute ceiling global rather than per client — a self-inflicted outage rather than a limiter. It is only as trustworthy as the proxy in front, and needs verifying against the deployed instance.
...
- **The SSRF guard is TOCTOU.** `WebhookTargetValidator` (v1.9) resolves the host immediately before connecting, but `RestClient` resolves again to open the socket, so the record could in principle change between the two. Pinning the resolved address and sending an explicit `Host` header is the complete fix.
- `POST /api/v1/merchants` returns 200; REST convention is 201 with a `Location` header. There is also no password-change endpoint.
...
| **v1.9** | **25** | **SSRF guard at delivery time, failing closed by default; `GET /api/v1/merchants/me`; signup moved off the entity onto `CreateMerchantRequest`, closing a privilege escalation that let an unauthenticated POST grant itself ADMIN; `Retry-After` on 429s; `PUT /merchants/{id}` moved onto `UpdateMerchantRequest`, completing the rule that no controller binds a request body to an entity, and fixed to return 404 rather than 200-with-no-body for a missing id; `DELETE /merchants/{id}` now answers 204 or 404 instead of 200 with the body `false`, so no service method in the merchant package signals absence by return value any more; per-IP rate limiting on `/auth/login`, closing the last endpoint that could be brute-forced without a ceiling.** |